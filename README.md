# LinguaSync Pro v7.1 — AI 视频同声传译

Chrome 扩展，在任何视频网页上浮现 AI 同声传译悬浮窗。播放外语视频时自动识别语音、实时翻译为中文，字幕叠加在视频画面上。内置知识胶囊术语解释、实时思维导图、开小差补救、流式问答伴侣、会话统计面板、回放锚点、自定义术语表等智能辅助功能。
...
- **会话统计面板 (Session Stats)** — 实时追踪翻译句数、总词数、平均语速（wpm）、会话时长、自纠正次数、问答次数，语速趋势 SVG 折线图 + 高频术语 Top 10
- **回放锚点 (Playback Anchors)** — 每条会议记录绑定视频时间点，点击任意一句即可跳转视频到对应位置重听，暂停状态自动播放
- **自定义术语表 (Custom Glossary)** — 用户可添加自定义术语翻译对（如 `Transformer → 变换器架构`），翻译时优先匹配用户词库，`chrome.storage` 持久化
...
- **四标签页** → **六标签页** — 会议记录 / 智能待办 / 思维导图 / 问答 / 统计 / 术语，蓝色指示器跟随切换
...
### 会话统计面板 (Session Stats)

切换到「统计」标签页，查看当前会话的实时数据概览：

- **六项核心指标** — 翻译句数、总词数、平均语速（wpm）、会话时长（分钟）、自纠正次数、问答次数，以卡片网格展示
- **语速趋势图** — SVG 折线图展示实时语速变化，每 30 秒采样一次，上限 500 wpm，最多保留 60 个采样点
- **高频术语 Top 10** — 按出现频率排序展示当前会话中最常出现的专业术语，带频率角标
- **刷新按钮** — 右上角可随时手动刷新统计数据

统计在同传启动时开始记录，`SessionStats` 类轻量运行，对性能零影响。

### 回放锚点 (Playback Anchors)

每条会议记录自动绑定翻译时的视频播放时间点：

- **播放三角图标** — 有时间戳的记录左侧显示蓝色 `▶` 图标，表示该句可跳转
- **一键跳转** — 点击任意会议记录条目，视频立即 seek 到对应时间点，暂停状态会自动开始播放
- **高亮反馈** — 跳转后该条目出现 1.5 秒蓝色脉冲动画，确认跳转成功
- **适用场景** — 回顾没听懂的片段、反复精听关键段落、对照原文和译文验证翻译质量

### 自定义术语表 (Custom Glossary)

切换到「术语」标签页，管理个人词库：

- **添加术语** — 两个输入框分别填入术语原文和自定义翻译（如 `Transformer` → `变换器架构`），点击 `+` 或按 `Enter` 添加
- **翻译优先匹配** — 翻译前自动扫描原文，命中的自定义术语以 `[Transformer=变换器架构]` 格式注入翻译上下文，API 优先采用用户指定的翻译
- **持久化存储** — 术语表通过 `chrome.storage.local` 保存，关闭浏览器后重新打开仍然保留
- **管理操作** — 每条术语右侧有红色 `×` 删除按钮，术语以紫色标签 + 绿色翻译箭头展示
...
│  ├─ SessionStats (语速采样 + 指标统计 + SVG 图表)     │
│  ├─ CustomGlossary (用户词库 + storage 持久化)        │
│  ├─ FloatingWidget (六标签页悬浮控制面板)              │
│  ├─ 回放锚点 (videoTime 绑定 + seek 跳转 + 高亮)      │
...
**Q: 如何回放某句没听清的内容？**
点击会议记录面板中对应的句子（带蓝色 `▶` 图标的条目），视频会自动跳转到该句对应的播放时间点。如果视频处于暂停状态会自动开始播放。

**Q: 自定义术语表如何使用？**
展开面板后切换到「术语」标签页，输入术语原文（如 `Transformer`）和期望的翻译（如 `变换器架构`），点击 `+` 或按 `Enter` 添加。之后翻译时如果原文中出现该术语，会优先使用你指定的翻译。术语表保存在浏览器本地，重启后仍然保留。

**Q: 统计面板的数据什么时候清空？**
统计在同传启动时重新开始记录，每次点击「开始同传」会重置计时器和采样数据。翻译句数、词数、术语频率等指标在整个同传会话期间持续累计。
